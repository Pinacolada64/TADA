...
Not started yet. Today a real Commodore 128 connects the same way a C64
does: `Translation.PETSCII`, one of the "Commodore 128" Client Type
presets (`commands/prefs.py`'s `_client_type_presets()`, 40x25 or 80x25),
still driven by the same `tada-client.asm` as a real C64 -- there is no
dedicated 128-mode binary. This file is a running list of what a real
128-native client could take advantage of that the C64 client can't, to
scope out before starting one. Escape-code citations are from
