...
- ✅ **EXAMINE \<player\>** — inspect another player in the same room: experience
  ...
  scale.
- ✅ **EXAMINE (no target) lists other room-mates too** — each other player
  sharing the room is tagged "watches you.." or, if unconscious, "lies
  unconscious." (`SPUR.MISC3.S:539–559`'s `ply.locD`/`ply.loc3`, called with
  `ch=2` from EXAMINE — `ch=0` is LOOT's own call to the same routine, see
  the LOOT command entry below; sentence-cased per this port's convention).
...
- ✅ **Unconscious state (duel loss)** — `SPUR.DUEL2.S`'s "uncon" subroutine: losing a duel sets `PlayerFlags.UNCONSCIOUS` + `player.defeated_by` (`combat/duel.py`'s `DuelSession._end()`), tagged "(Unconscious)" in room listings (`simple_server.py`), blocks being re-challenged to a duel ("You can't duel `<name>` -- they're unconscious!", `combat/duel.py`'s `_send_challenge()`), and changes GET's player-target message to "won't fit in your sack.." instead of "skuttles out of reach!" (`commands/get.py`). Clears at the player's next login with SPUR's own wake-up line, extended to name the opponent (`logon_events/unconscious_wake.py`, ported from `SPUR.LOGON.S`'s `rd.user`, lines 277-281). EXAMINE (no target) also lists room-mates as "watches you.."/"lies unconscious." (see the Examine section above). **Not yet implemented**: carrying an unconscious guild member (Strength-gated, `messages.json`/`TODO.md` -- see the Honor/death-mechanics TODO); `SPUR.MAIN.S:443`'s "lies here unconscious!" line, which lives inside the `autoduel` subroutine describing an *offline* character's saved-stat "shell" standing in a room -- that whole offline-shell display mechanism doesn't exist in this port (see AUTODUEL in the Honor/death-mechanics TODO), so this line isn't a standalone fix, it's part of that larger unbuilt feature
