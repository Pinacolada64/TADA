...
- ✅ **LURK mode** — `[L]urk` combat menu option; requires at least one living ally ("No allies — no LURK!" if none); costs Honor (base 2, +1 Assassin, +1 HP>20, -1 HP<10, -1 more HP<5, -1 if the swing won't fire; no deduction if Honor isn't strictly greater than the cost); a loaded, non-STORM, non-LIGHT-named projectile/energy weapon fires over the ally's head at a -2 to-hit/-2 damage penalty (`player_attacks(is_lurking=True)`, ease-of-use fast path disabled); any other weapon (melee, empty ammo weapon, or LIGHT SABRE) skips the player's own swing entirely and only the allies attack (`SPUR.COMBAT.S:82–96`, `combat/lurk.py` `resolve_swing()`, `has_living_ally()`). While lurking, the monster's counter-attack is guaranteed to redirect off the player and onto a random living ally instead — the ally takes the same damage the player would have (`SPUR.COMBAT.S:247–262, 324–341`, `lurk.a`/`m.a1`, `combat/lurk.py` `try_redirect_to_ally()`, `combat/engine.py` `CombatSession._is_lurking_this_round`). Not ported: SPUR's Honor-gated ally morale-failure roll where a lightly-wounded ally flees instead of taking the redirected hit (`SPUR.COMBAT.S:328–330, 341`)
