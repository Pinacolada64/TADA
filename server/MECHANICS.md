...
- ✅ **ORDER command** — deploy up to 3 owned servants as Point Man / Flank Guard / Rear Guard; every owned servant must be placed somewhere (a slot can be left NONE if you own fewer than three), matching SPUR's "You didn't deploy ALL your servants!" retry loop; position persists across save/load (`bar.ally_data.Ally.position`, `party.py` to_json/from_json) (`SPUR.MISC2.S`, `commands/order.py`). **Not yet ported**: SPUR.MISC4.S's "tactical" ambush check, where a monster that spots the party first picks one of these three slots at random and that servant either shouts a warning and holds or gets caught off guard — today ORDER's positions are a roster/flavor display only, with no combat effect yet.
