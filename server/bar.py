...
def blue_djinn():
    ...
    if flag['expert_mode'] is False:
        print(f"For a price, {character} can attack other players.")
        blue_djinn_menu()
    print(f'{character} sits behind the table.')
    while True:
        ...
        if command == '?':
            ...
            print(f"{character} looks amused.")
            continue
def blue_djinn_menu():
...
def skip():
    ...
    while True:
        command, last_command = prompt(f'"What\'ll ya have, {player_name}?" ')
        if command == 'h':
