...
def _menu_options_lines(ctx) -> list[str]:
    ...
    t.add_row(['|reverse_on|<#>|reverse_off|', 'jump straight to reply #'])
    t.add_row([f'|reverse_on|{ctx.player.return_key}|reverse_off|', 'read the next message'])
    t.add_row(['|reverse_on|pm|reverse_off|', 'toggle Prompt Mode'])
