...
class ReadCommand(Command):
    ...
    async def execute(self, ctx: GameContext, *args) -> CommandResult:
        ...
        # first and reuses commands/examine.py's exact flavor text (the same
        # "small brass plaque" message 'examine statue' shows) rather than
        # duplicating it here.
        if args:
            target = ' '.join(args).lower()
            if 'statue' in target:
                from commands.get import _room_available_items
                from commands.examine import _examine_item
