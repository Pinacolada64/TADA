...
Two sibling sub-events of this same dispatcher aren't in this package
at all -- they live in `ally_events/` instead, since they're ally
mechanics first and world-event-roll encounters second:

- An ally finding gold (`al.find`) is `ally_events/__init__.py`'s
  `try_ally_find_gold()` (own once-per-day tag `'AYF'`, own flat 5%
  chance), wired in alongside this package's modules at the same
  `simple_server.py` call sites.
- A weakened ally dying of neglect (`dead.al`) is
  `ally_events/starvation.py`'s `try_encounter()` -- unless it's a
  GOD/GODDESS ally, which deserts instead (skip's distinction), or
  ELITE, which just endures (not from SPUR). Honor/Wisdom/Intelligence
  penalties, battle.log entry. No water/vacuum gate, no once-per-session
  gate -- can fire repeatedly. 0.3% per move (2% world-event roll × 15%
  slice), wired in at the same `simple_server.py` call sites.
...
| An ally finding gold | 15% (`z<45`, `al.find`) | 17% (`z<49`) | ✅ Pre-existing -- `ally_events/__init__.py`'s `try_ally_find_gold()`, not part of this package |
| An ally's death | 15% (`z<60`, `dead.al`) | Called unconditionally every roll, not a slice at all | ✅ `ally_events/starvation.py` (master's stat penalties, skip's GOD/GODDESS "deserts instead of dying" distinction), not part of this package |
...
- `little_girl.py`'s, `meteor.py`'s, and `djinn_sighting.py`'s
  ...
  players witnessing an event) -- `ally_events/starvation.py` has the
  same addition, though it lives outside this package.
