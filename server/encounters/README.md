...
`little_girl.py` and `meteor.py` are both slices of the *same* dispatcher
in the original source (`SPUR.MAIN.S:239`'s 2% per-move roll, then a d100
sub-roll across six sub-events in `SPUR.MISC6.S`'s `random`/`no.test`
labels):

| Sub-event | Master's slice | Status |
|---|---|---|
| Galadriel | 15% (`z<15`) | ✅ Implemented — but as its own quest trigger (quest #8, `quests/README.md`), not wired into this dispatcher |
| Meteor | 15% (`z<30`) | ✅ `meteor.py` |
| An ally's death | 15% (`z<45`) | Not built |
| An ally finding gold | 15% (`z<60`) | Not built |
| The Enforcer | 20% (`z<80`) | Not built — see `TODO.md`'s "7/15/26" `ys$` inventory for the full trace (`*enf` token, a named NPC duel against "THE ENFORCER") |
| Little Girl | 20% (`z>=80`) | ✅ `little_girl.py` |

Since only two of the six sub-events exist so far, `little_girl.py` and
`meteor.py` each roll their own **flat composite share** independently
(2% × their slice) instead of sharing one real dispatcher. This means —
unlike the original, where only one sub-event can fire per successful
roll — both could in principle trigger on the same move right now. Once
a third sub-event gets built, replace both flat rolls with a real
shared dispatcher module (e.g. `encounters/world_events.py`) that does
the 2% roll once, then the d100 sub-roll, and routes to whichever
`try_*` function corresponds to the result — cleaner, and restores
mutual exclusivity to match the source.

The skip branch's version of this dispatcher (`SPUR.MISC6.S:140-148`,
skip) is meaningfully different — it adds a seventh sub-event (a random
Blue Djinn encounter, distinct from `bar/blue_djinn.py`'s hit-contract
mechanic), calls `dead.al` unconditionally every time rather than as one
of the six slices, and per-event `ys$` gates are checked *inline during
the roll* so a roll that lands on an already-seen event's slice spills
over into the next untried one instead of just doing nothing that move.
Worth designing the real dispatcher off skip's version rather than
master's, if/when this gets built — it's the more polished design, even
though master is otherwise the default source of truth in this project
(see `quests/README.md`'s sourcing rule).
...
- `little_girl.py`'s and `meteor.py`'s `ctx.send_room()` bystander
  broadcasts (SPUR has no concept of other players witnessing an event).
