...
# Sender name for mail generated by game logic rather than typed by a real
# player (e.g. the "while you were unconscious, you were robbed" notice --
# see commands/loot.py -- ported from SPUR.MISC3.S's own dm$+"mail" write,
# which had no equivalent concept of a sender at all since it was a direct
# file append, not a MAIL-command message). "SPUR" was Ryan's suggestion:
# reads naturally as the game itself speaking, and doubles as a nod to the
# original game this is a port of.
SYSTEM_SENDER = 'SPUR'
...
def add_system_message(player_name: str, body: str) -> None:
    """Send *player_name* a game-generated notice (from SYSTEM_SENDER)
    rather than one from another player -- same record shape, just a
    fixed, non-player sender so it reads unambiguously as the game
    talking, not a fellow adventurer."""
    add_message(player_name, SYSTEM_SENDER, body)
