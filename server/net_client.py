...
class Client:
    """Information about clients connecting to the TADA server."""
    ...
    def connect(self, host, port, server_id, server_key, protocol_version, translation: "Translation"):
        ...
        try:
            ...
            from net_common import Mode
            init_data = {
                'mode': Mode.init,           # Server mode (must initially be 'init')
            ...
            while self._connected and not self._shutdown_event.is_set():
                try:
                    ...
                    try:
                        ...
                        try:
                            message = from_jsonb(self.client_socket)
                            if message is None:
...
def main():
    ...
    test_mode = getattr(server, 'Test', None)
    if test_mode:
        key = test_mode.key
