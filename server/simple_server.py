...
class Init:
    ...
    def __init__(self, server_id="test_server", server_key="test_key", protocol_version=1,
                 translation="utf-8", type=MessageType.init):
...
class Server:
    ...
    async def _handle_login(self, reader, writer, addr, client):
        from commands import new_player, connect
        """
        # Register commands related to login
        command_processor.register_command(new_player.NewPlayerCommand)
        command_processor.register_command(connect.ConnectCommand)
        command_processor.register_command(guest.GuestCommand)
        """
        ...
        # This loop is now handled in handle_connection's main loop
        await self.receive_message(reader)  # Just to ensure we wait for the next message
    ...
    async def handle_login_mode(self, client, in_message, writer):
        ...
        if in_message.lines and isinstance(in_message.lines, list):
            for line in in_message.lines:
                ...
                # TODO: handle _execute() method in registered commands:
