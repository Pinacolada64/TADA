#!/usr/bin/env python3
"""Quick live check: does the class/race bonus report show after accepting rolled stats?"""
import asyncio
import json
...
async def main():
    ...
    script = [
        ...
        ('7', 'login'),          # class: Archer
        ('1', 'login'),          # race: Human
        ('c', 'login'),          # guild
        ('y', 'login'),          # accept rolled stats  <-- watch this response
        ('quit', 'login'),       # bail out, don't need to finish
        ('A', 'login'),          # abandon
    ]
