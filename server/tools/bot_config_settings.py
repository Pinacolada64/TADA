...
async def main():
    ...
    await _send(writer, {'lines': ['connect botdummy puppy123'], 'mode': 'login'})
    await _recv_all(reader, timeout=4.0)

    for cmd in ['help config', 'config victory_type', 'config victory_type both',
                'config victory_type', 'config session_time_limit_minutes 200',
                'config not_a_real_key', 'config port 9999']:
        print(f"\n=== -> {cmd!r} ===")
        await _send(writer, {'lines': [cmd], 'mode': 'game'})
