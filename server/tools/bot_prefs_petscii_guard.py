#!/usr/bin/env python3
"""Reproduce server/hardcopy.0's scenario: pick 'Commodore 128, 80 col'
from an ANSI-connected (tada_client.py-like) session via PREFS, and
confirm no raw PETSCII bytes leak into the transcript."""
import asyncio
import json
...
def _print(msgs):
    for m in msgs:
        ...
        if isinstance(lines, str):
            lines = [lines]
        for l in lines:
            print(repr(l) if any(ord(c) < 32 and c not in '\n\t' for c in l) else l)
        if m.get('prompt'):
            print(f'  [{m["prompt"]}]')
...
async def main():
    ...
    script = [
        ...
        ('prefs', 'game'),
        ('t', 'game'),
        ('3', 'game'),      # Commodore 128, 80 col -- the hardcopy.0 scenario
        ('', 'game'),       # exit prefs
        ('quit', 'game'),
        ('Y', 'game'),
    ]
